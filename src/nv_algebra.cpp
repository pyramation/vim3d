...
{
    float oow = nv_one / (v.x * M.a03 + v.y * M.a13 + v.z * M.a23 + M.a33);
...
mat3& tangent_basis(mat3& basis, const vec3& v0, const vec3& v1, const vec3& v2, const vec2& t0, const vec2& t1, const vec2& t2, const vec3 & n)
{
    ...
    // tangent...
    float oonorm = nv_one / sqrtf(basis.a00 * basis.a00 + basis.a01 * basis.a01 + basis.a02 * basis.a02);
    basis.a00 *= oonorm;
    basis.a01 *= oonorm;
    basis.a02 *= oonorm;
...
{
    float s, t, sc, tc;
    s = (float(x) + nv_zero_5) / float(cubesize);
    t = (float(y) + nv_zero_5) / float(cubesize);
    sc = s * nv_two - nv_one;
    tc = t * nv_two - nv_one;
...
float nv_area(const vec3& v1, const vec3& v2, const vec3& v3)
{
    vec3 cp_sum;
    vec3 cp;
    cross(cp_sum, v1, v2);
...
float nv_perimeter(const vec3& v1, const vec3& v2, const vec3& v3)
{
    float perim;
    vec3 diff;
    sub(diff, v1, v2);
...
float nv_find_in_circle(vec3& center, const vec3& v1, const vec3& v2, const vec3& v3)
{
    float area = nv_area(v1, v2, v3);
    // if the area is null
    if (area < nv_eps)
    {
        center = v1;
    ...
    float oo_perim = nv_one / nv_perimeter(v1, v2, v3);
...
float nv_find_circ_circle( vec3& center, const vec3& v1, const vec3& v2, const vec3& v3)
{
    vec3 e0;
    vec3 e1;
    float d1, d2, d3;
